  ...
  return {
    ...
    compareOnKey: function compareOnKey(key, rule) {
      return function (a, b) {
        if (key === "name" || key === "job" || key === "email" || key === "country") {
          var comparison = 0;
          var fieldA = a[key].toUpperCase();
          var fieldB = b[key].toUpperCase();
    ...
    search: function search(value) {
      if (value.length > 1) {
        ...
        this.items = fuse.search(value).map(function (elem) {
          return elem.item;
        });
      } else {
        this.items = data;
    ...
    changePage: function changePage(page) {
      if (page >= 1 && page <= this.pagination.lastPage) {
        ...
        var lastPage = Math.ceil(total / this.view) || 1;
        var from = (page - 1) * this.view + 1;
        var to = page * this.view;
    ...
    showPages: function showPages() {
      var pages = [];
      var from = this.pagination.currentPage - Math.ceil(this.offset / 2);
